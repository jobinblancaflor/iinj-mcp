	...
	*/
	...
	function census()
	{
		...
		if ($this->flexi_auth->is_logged_in()) 
		{
			...
			if ($this->input->post('register_census'))
			{		
				$path= "uploads/";
				...
				else
				{
					...
					$table="census";
					$data = array(
								'census_first_name' => $this->input->post('census_first_name'),
								'census_middle_name' => $this->input->post('census_middle_name'),
								'census_last_name' => $this->input->post('census_last_name'),
								'census_gender' => $this->input->post('census_gender'),
								'census_birthday' => $this->input->post('census_birthday'),
								'census_congregation' => $this->input->post('census_congregation'),
								'census_designation' => $this->input->post('census_designation'),
								'census_country' => $this->input->post('census_country'),
								'census_phone' => $this->input->post('census_phone'),
								'church_status' => $this->input->post('church_status'),
								'census_profile' => $path.$data['file_name']
					);
					$this->insert_model->insert($table,$data);
					
					
					//exit();
					unset($_POST);
					$this->data['message'] = "<p class='status_msg'>You have successfuly insert census.";
					//$this->session->set_flashdata('message', $this->flexi_auth->get_messages());				 
					// redirect('auth_admin/census');
					
			//$this->load->view('demo/admin_examples/census_view', $this->data);
				}		
			}
			
			$this->load->model('select_model');
			$this->load->model('custom_model');
			$this->data['congregation']= $this->custom_model->get_congregations();
			$this->data['designation']= $this->custom_model->get_designation();
			
			$statement = "Select country_name from country";
			$this->data['country']= $this->select_model->select_statement($statement);
			
			//$this->data['message'] = "You have successfuly insert census.";
			
			// Get any status message that may have been set.
			$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
			
			$this->load->view('demo/admin_examples/census_add_view', $this->data);
		}else {
			redirect('auth');
		}
	}
	
	function edit_census($census_id)
	{
		// Redirect user away from registration page if already logged in.
		if ($this->flexi_auth->is_logged_in()) 
		{
			
			//$census_id = $this->uri->segment(3);
			
			// If 'Registration' form has been submitted, attempt to register their details as a new account.
			if ($this->input->post('update_census'))
			{
				...
				{			
					$data = $this->upload->data();
				// $this->load->view('demo/admin_examples/census_table', $this->data);
			//$this->load->view('demo/admin_examples/census_view', $this->data);
				}	
				
				if($data['file_name']==""){
					$image = $this->input->post('old_profile');
				}else{
					...
					$data = array(
								...
								'church_status' => $this->input->post('church_status'),
								'census_country' => $this->input->post('census_country'),
								'census_profile' => $image
					);
					$column_id = "census_id";
					$this->update_model->update($table,$column_id,$census_id,$data);
					
					
					//exit();
					unset($_POST);
					$this->data['message'] = "<p class='status_msg'>You have successfuly updated census.";
					$this->session->set_flashdata('message', $this->flexi_auth->get_messages());
				
				redirect('auth_admin/table_census');
			}
			...
			$this->data['congregation']= $this->custom_model->get_congregations();
			$this->data['designation']= $this->custom_model->get_designation();
			$this->data['census_data']= $this->select_model->select_where("census",$census_id);
			
			$statement = "Select country_name from country";
			$this->data['country']= $this->select_model->select_statement($statement);
			//print_r($this->data['census_data']);
			//$this->data['message'] = "You have successfuly insert census.";
			
			// Get any status message that may have been set.
			$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
			
			$this->load->view('demo/admin_examples/census_update_view', $this->data);
		}else {
			redirect('auth');
		}
	}
	function view_census()
	{
		// Redirect user away from registration page if already logged in.
		if ($this->flexi_auth->is_logged_in()) 
		{
			...
			$this->load->model('select_model');
			$this->data['census_data']= $this->select_model->select_where("census",$census_id);
			$query = "Select * from attendance where census_id =".$census_id." and status = 1";
			$this->data['attendance']= $this->select_model->select_statement($query);
			
			$this->load->view('demo/admin_examples/census_info_view', $this->data);
		}else {
			redirect('auth');
		}
	}
	public function search_census() 
    {
        ...
        $total_records = $this->select_model->get_total_search($search);
 
        if ($total_records > 0) 
        {
            // get current page records
            $params["results"] = $this->select_model->get_current_page_records_search($limit_per_page, $start_index,$search);
             
            $config['base_url'] = base_url() . 'index.php/auth_admin/search_census';
			...
			$config['cur_tag_open'] = '<li class="active"><a class="current" href="#">';
        ...
        //print_r($params);
        $this->load->view('demo/admin_examples/census_table', $params);
	...
	public function delete_census() 
    {
		...
		$this->update_model->update($table,$id_name,$id,$data);
        ...
        // init params
        $params = array();
		$params['message'] = "<p class='status_msg'>You have successfuly deleted census.";
        $limit_per_page = 15;
	  // $start_index = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
	   $start_index = 0;
        $total_records = $this->select_model->get_total();
 
        if ($total_records > 0) 
        {
            // get current page records
            $params["results"] = $this->select_model->get_current_page_records($limit_per_page, $start_index);
             
            $config['base_url'] = base_url() . 'index.php/auth_admin/table_census';
            $config['total_rows'] = $total_records;
            $config['per_page'] = $limit_per_page;
            $config["uri_segment"] = 3;
			$config["full_tag_open"] = '<ul class="tsc_pagination">';
	...
	public function congregation_census($congregation_id) 
    {
		...
		foreach($congregation_data as $congregation_row)
		$congregation = $congregation_row['congregation'];
        $this->load->library('pagination');
        // init params
        $params = array();
        $limit_per_page = 15;
        $start_index = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $total_records = $this->select_model->get_total_congregation_census($congregation);
 
        if ($total_records > 0) 
        {
            // get current page records
            $params["results"] = $this->select_model->get_current_page_records_congregation_census($limit_per_page, $start_index,$congregation);
             
            $config['base_url'] = base_url() . 'index.php/auth_admin/congregation_census';
            $config['total_rows'] = $total_records;
            $config['per_page'] = $limit_per_page;
            $config["uri_segment"] = 3;
			$config["full_tag_open"] = '<ul class="tsc_pagination">';
